...
def brand_upload(request):
    if request.method == 'POST':  # this means the form has data
        ...
        if form.is_valid():  # check if it is valid
            ...
            messages.success(
                request,
                'Your brand has been added! Please wait\
                to be approved to Upload A product Usual wait Time: 24 hours.')
            ...
            messages.success(request, 'Error in creating your product,\
...
def product_upload(request):
    ...
    if request.method == 'POST':  # this means the form has data
        form = Product_form(
            request.POST,
            request.FILES)  # get the form and it data
        if form.is_valid():  # check if it is valid
            ...
            messages.success(
                request,
                'Error in creating your product, the form is not valid!')
            return render(
                request,
